        ...
        <section class="content">
          <section class="build-standard">
            <ul class="space-between">
              <li v-if="index !== 3" v-for="(item,index) in orderControl" @click="setOrder(item,index)" >
                {{item.title}}
            ...
            <mt-loadmore 
              ...
              ref="loadmore">
                  <ul class="space-left" v-for="(item,index) in dealLineNum(dataList,4)" v-if="dataList.length > 0">
                    ...
                    >
        <!--                 <div :style="{background:'url('+bgImg(dataList[index * 4 + cindex]) +')',backgroundSize:'cover',backgroundRepeat:'no-repeat',backgroundPosition:'center',border:'0.01rem solid #ddd'}">

                        </div>
                        <p v-if="dataList[index * 4 + cindex].Title.length > 10" 
                          class="two-line" style="line-height: .35rem">{{dataList[index * 4 + cindex].Title}}</p>
                        <p v-else class="one-line" style="line-height: .7rem">{{dataList[index * 4 + cindex].Title}}</p>
